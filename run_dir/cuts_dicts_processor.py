...
for processname in processnames:
    for masslabel in masslabels:
        ...
        pattern1 = re.compile(rf'{masslabel}.*\.pkl$')
...
#        print(Nobs)
        ...
        if Nobs != 0:
            ...
            br_sens_dict_uncertainty[processname][masslabel] = br_sens_dict[processname][masslabel] *(np.sqrt(Nobs)/Nobs)
        else:
            br_sens_dict[processname][masslabel] = 1. # is 0 but change to one as cant resolve any branching ratio
...
#        print(Nobs)
        ...
        if Nobs != 0:
            ...
            br_sens_dict_new_uncertainty[processname][masslabel] = br_sens_dict[processname][masslabel] *(np.sqrt(Nobs)/Nobs)

        else:
            br_sens_dict_new[processname][masslabel] = 1. # is 0 but change to one as cant resolve any branching ratio
            print("no events observed in ", processname, " for mass ", masslabel)
#print("br_sens_dict: ")
#print(br_sens_dict)
...
#    print(masstoplot)
    ...
    brtoplotUncertainty = list(br_sens_dict_new_uncertainty[processname].values())
    ...
    plt.errorbar(masstoplot, brtoplot, yerr = brtoplotUncertainty, color=plotstyle_dict[processname]['colour'])
plt.title("BR sensitivity")
plt.legend()
