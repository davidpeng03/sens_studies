...
if (check_ctau == True):
    ...
    #r_expo_end_vertex = [math.sqrt(x**2 + y**2 + z**2) for x, y, z in end_vertex_llp_all]
...
#        gamma = 1./math.sqrt(1. - beta**2)
    ...
    if (make_ctau_plot == True):
        ...
        def exp_decay(x, a, b):
            return a * np.exp(-b * x)
        histogram = plt.hist(ctau_dec_llps, bins=fig_ctau_bins, color='#6fa3f7', label='HNLs',density=True)
        frequency = histogram[0]
        bin_edges = histogram[1]
        bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
        popt, pcov = sp.optimize.curve_fit(exp_decay, bin_centers, frequency)
        a_fit, b_fit = popt
        ...
        param_text = f"$y = {a_fit:.2f} e^{{-{b_fit:.2f}x}}$"
        plt.text(0.6 * max(bin_centers), 0.8 * max(frequency), param_text, fontsize=12, color="red")
        drate = 1/b_fit
        plt.text(0.6 * max(bin_centers), 0.75 * max(frequency), f"Hence from fit  decay rate is {drate}")

        hbar = 6.582e-25
        t = hbar/llp_width
        t = 3e8* t
        plt.text(0.6 * max(bin_centers), 0.7 * max(frequency), f"c times decay rate from llp width is {t}")
        plt.axvline(drate, color='#2a9d8f', linestyle='dashed', linewidth=1,label='ctau from fit')
        plt.axvline(t, color='#9a6aff', linestyle='dashed', linewidth=1,label='ctau from llp width')
        ...
        plt.axvline(avg_ctau, color='#e07b00', linestyle='dashed', linewidth=1,label='hepmc mean ctau')
        plt.axvline(auto_ctau, color='#FF6F61', linestyle='dashed', linewidth=1,label='MG auto ctau')
        plt.xlabel('ctau (decay) (m)')
        plt.ylabel('Density')
        plt.title(f'Decay ctau of HNLs (VeN1={llp_Ve}, mN1={llp_mass} GeV)')
        ...
        plotSuffix = now.strftime("%Y%m%d_%H_%M_%S")
        plt.savefig(f"ctau{plotSuffix}.png")
...
def makehistograms(parameter,title=False,density=False):
    plt.hist(parameter, bins = 50, density = density )
    plt.title(title)
    plt.ylabel('counts', fontsize = 14)
    plt.xlabel('bottom text', fontsize = 14)
    plt.show()
    plt.savefig(f"{title}.png")

def ptcuttings(parameter,cuttings):
    cut_parameter_list = []
    firstcut = jet_pt<cuttings[0]
    cut_parameter_list.append(parameter[firstcut])
    
    for i in range(len(cuttings)-1):
            upper_cut = jet_pt>cuttings[i]
            lower_cut = jet_pt<cuttings[i+1]
            full_cut = upper_cut*lower_cut
            cut_parameter_list.append(parameter[full_cut])
    finalcut = jet_pt>cuttings[len(cuttings)-1]
    cut_parameter_list.append(parameter[finalcut])

    return cut_parameter_list


"""makehistograms(expo_end_vertex_llp_all,"expo_end_vertex_llp_all")

makehistograms(E_llp_all,"E_llp_all")

makehistograms(production_vertex_llp_all,"production_vertex_llp_all")

makehistograms(px_llp_all,"px_llp_all")

makehistograms(pt_llp_all,"pt_llp_all")

makehistograms(theta_llp_all,"theta_llp_all")
"""
...
if (do_post_checks == True):
    ...
    plt.axvline(x = 15.5266, color = 'black', linestyle = '--', label="ANUBIS" ) 
    plt.axvline(x = -15.5266, color = 'black', linestyle = '--', label="ANUBIS") 
    ...
    plt.axvline(x = 20, color = 'black', linestyle = '--', label="ANUBIS") 
    ...
    plt.axvline(x = -26.5098, color = 'black', linestyle = '--', label="ANUBIS" ) 
    plt.axvline(x = 26.3521, color = 'black', linestyle = '--', label="ANUBIS") 
    ...
    m_llp_all_sum = sum(m_llp_all)
    children_summass_llp_all_sum = sum(children_summass_llp_all)
    px_llp_all_sum = sum(px_llp_all)
    pz_llp_all_sum = sum(pz_llp_all)
    pt_llp_all_sum = sum(pt_llp_all)
    print(f"total x momentum:{px_llp_all_sum}")
    print(f"total z momentum:{pz_llp_all_sum}")
    print(f"total transverse momentum:{pt_llp_all_sum}")
    print(f"llp sum mass:{m_llp_all_sum}")
    print(f"children sum mass:{children_summass_llp_all_sum}")
    if m_llp_all_sum == children_summass_llp_all:
        print("nice")
