...
### specifically in this version of the decay_pos_compare.py script, introducing the h5 files as direct comparison. Loading h5 files output by save_ (in the same way as load in apply_) and check that the expo_decay distances there are the same when averaged as the avg dec pos saved in txt files.

## this is to debug how trustworthy the geometry cuts are, after noticing strange behaviour in the efficiency plots. 
