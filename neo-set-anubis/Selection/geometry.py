...
def is_within_cavern(decay_pos):
    if not isinstance(decay_pos, (list, np.ndarray)) or len(decay_pos) != 3:
        raise ValueError(f"Invalid decay_pos format: {decay_pos}")

    x, y, z = decay_pos # decay_pos[0], decay_pos[1], decay_pos[2]
    # Check if within rectangular cavern bounds
    if -cavern_x_max <= x <= cavern_x_max and \
       y > atlas_t_max and \
       -cavern_z_max <= z <= cavern_z_max:
        # Check if below the cavern curvature
        curvature_y_max = np.sqrt(cavern_r_curvature**2 - x**2) + (cavern_sagitta + cavern_y_max - cavern_r_curvature)
        if y < curvature_y_max:
            return True  # Point is inside the cavern and under the curvature

    return False  # Point is outside the cavern
...
def in_cavern(r, eta, phi):
    decay_pos = list(to_cartesian(r, eta, phi))
#    decay_pos = [pos_mm / 1000. for pos_mm in decay_pos_mm]
...
def ceil_intersection(eta, phi, decay_pos): # input is cartesian 
#    decay_pos = [pos_mm / 1000. for pos_mm in decay_pos_mm]
...
#        return False
    ...
    if jet_y < 0:
        print("jet_y < 0 in ceiling_stat")
        return False
    

    # Check if intersection is within z-bounds
    if jet_z > cavern_z_max or jet_z < -cavern_z_max:
        print("jet_z > cavern_z_max or jet_z < -cavern_z_max in ceiling_stat")
        return False
    ...
    if jet_x > ceil_length or jet_x < -ceil_length:
        print("jet_x > ceil_length or jet_x < -ceil_length in ceiling_stat")
        return False

    # Must be wihtin the ceiling bounds
    print("Within ceiling bounds in ceiling stat!!!!!!!!!!!!!!!!!")
    return True
