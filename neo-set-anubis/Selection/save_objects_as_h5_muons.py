...
llp_Vmu = samples_to_process_df.iloc[SampleY]['numixing#4']
...
hbar = 6.582e-25
c_light = 3.0e+8
auto_ctau = c_light * hbar / llp_width

HNLCalc_file = user_config['HNLCalc_file_muons']
HNLCalc_df = pd.read_csv(HNLCalc_file)
HNLCalc_ctau = HNLCalc_df.loc[(HNLCalc_df["Mass"] == llp_mass) & (HNLCalc_df["Coupling"] == llp_Vmu), "Lifetime"].values
...
filedescription = f"{llp_type}_{prod_decay}_mn1_{llp_mass}_vmun1_{llp_Vmu}_sample{SampleY}".replace('.','pt').replace('-','min')
...
#                llp_production_vtx_pos_m = [pos_mm / 1000. for pos_mm in p.production_vertex.position]
                ...
                boost = calculate_boost(calculate_pt(p.momentum[0],p.momentum[1]), eta_llp, llp_mass)
...
#                decay_pos_cartesian_original[0] += origin[0]
                ...
                boost_llp_all.append(boost)
                beta_llp_all.append(beta)
...
#        gamma = 1./math.sqrt(1. - beta**2)
    ...
    if (make_ctau_plot == True):
        ...
        plt.ylabel('Density')
        plt.title(f'Decay ctau of HNLs (VmuN1={llp_Vmu}, mN1={llp_mass})')
        plt.legend()
        plt.show()

    if (save_output_txt == True):

        llp_Vmu_label = str(llp_Vmu).replace('.','p')

        output_file = f"ctau_coupling{llp_Vmu_label}_{prod_decay}_muons.txt"
        file_exists = os.path.isfile(output_file)

        with open(output_file, "a") as file:
            # If the file does not exist, write the headers first
            if not file_exists:
                file.write("mass,hepmc_avgdecpos,hepmc_avgctau,madspin_autoctau,VmuN1,process,SampleY\n")
            file.write(f"{llp_mass},{avg_decay_pos},{avg_ctau},{auto_ctau},{llp_Vmu},{prod_decay},{SampleY}\n")
...
if (do_post_checks == True):
    ...
    print("r_llps")
    print(r_llps)
    r_llps_original = [math.sqrt(x**2 + y**2 + z**2) for x, y, z in end_vertex_llp_all]
    print("r_llps_original")
    print(r_llps_original)
#    plt.figure(figsize=(12,7))
    #plt.xlim(0,450)
...
#    plt.title(f'x histogram HNLs, SampleY={SampleY}, mass={llp_mass}, coupling={llp_Vmu}')
...
#    plt.ylabel('frequency')
#    plt.title(f'y histogram HNLs, SampleY={SampleY}, mass={llp_mass}, coupling={llp_Vmu}')
...
#    plt.ylabel('frequency')
#    plt.title(f'z histogram HNLs, SampleY={SampleY}, mass={llp_mass}, coupling={llp_Vmu}')

    plt.figure(figsize=(12,7))
    #plt.xlim(0,450)
    plt.hist(r_llps, bins=50, color='green', alpha=0.5, label='HNLs expo',density=True)
    plt.hist(r_llps_original, bins=50, color='pink', alpha=0.5,label='HNLs original hepmc',density=True)
    #sns.histplot(r_llps, bins=60, kde=False)
    plt.xlabel('r end vertex')
    plt.ylabel('frequency')
    plt.legend()
    plt.title(f'r HNLs, SampleY={SampleY}, mass={llp_mass}, coupling={llp_Vmu}')

    plt.figure(figsize=(12,7))
    #plt.xlim(0,450)
    plt.hist(boost_llp_all, bins=50, color='green', label='HNLs',density=True)
    #sns.histplot(r_llps, bins=60, kde=False)
    plt.xlabel('boost')
    plt.ylabel('frequency')
    plt.title(f'boost HNLs, SampleY={SampleY}, mass={llp_mass}, coupling={llp_Vmu}')
    ...
    plt.xlabel('beta')
    plt.ylabel('frequency')
    plt.title(f'beta HNLs, SampleY={SampleY}, mass={llp_mass}, coupling={llp_Vmu}')
...
plot_decay_positions = False

if (calculate_cuts == True):

    xyz_production_vertex_llps = []
    xyz_end_vertex_llps = []
    xyz_end_vertex_llps_cavern = []
#    yz_end_vertex_llps_expo = []
#    yz_production_vertex_llps_in_cavern = []
#    yz_end_vertex_llps_in_cavern = []
#    yz_end_vertex_llps_in_cavern_expo = []
#    yz_production_vertex_llps_in_cavern_ceil_stat = []
#    yz_end_vertex_llps_in_cavern_ceil_stat = []
#    yz_end_vertex_llps_in_cavern_ceil_stat_expo = []
...
#        if (data_llps['expo_end_vertex_llp_all'][j][0] != -1 and data_llps['n_decay_products_llp_all'][j] > 1): # shouldn't I also make sure the llp's status is the one that's long-lived here? Although I already do have the ndecayproducts>1 requirement -- which alone should already be the 1 final state LLP
        if(n_decay_products_llp_all[j] > 1):
...
#            print(f"r (mass = {masslabel}: ", r)
            decay_pos = (list(to_cartesian(r_expo_end_vertex[j], eta_llp_all[j], phi_llp_all[j]))) 
            # Add ATLAS offset:
            decay_pos[0] += origin[0]
            print("decay_pos = ", decay_pos)

            # prepare LLP decay positions to plot, no selections 
            xyz_end_vertex_llps.append((decay_pos[0], decay_pos[1], decay_pos[2]))
            xyz_production_vertex_llps.append((origin[0],0.,0.))

            llp_is_in_cavern = is_within_cavern(decay_pos)

#            llp_is_in_cavern = in_cavern(r_expo_end_vertex[j], eta_llp_all[j], phi_llp_all[j]) # in cavern already has atlas x axis offset included, as it's not added to the r_expo in this script 
#            llp_is_in_cavern = True # for testing 
#            llp_is_in_shaft = in_shaft(r_expo_end_vertex[j], eta_llp_all[j], phi_llp_all[j]) # we don't use the shaft 
            n_llps_all+=1
            n_llps_all_weighted+=weight_llp_all[j]
            llp_is_in_ceiling_direction = ceiling_stat(eta_llp_all[j], phi_llp_all[j], decay_pos) # this input contains the atlas offset and the function does not 
#            llp_ceil_int = ceil_intersection(eta_llp_all[j], phi_llp_all[j], decay_pos) # this input contains the atlas offset and the function does not 
#            print("llp_ceil_int = ", llp_ceil_int)
            if (llp_is_in_cavern and llp_is_in_ceiling_direction):
                print("llp is in cavern")
                xyz_end_vertex_llps_cavern.append((decay_pos[0], decay_pos[1], decay_pos[2]))
...
#                cartesian_r_end =  list(to_cartesian(r_end_vertex[j], eta_llp_all[j], phi_llp_all[j]))
...
#                   print("and passed ceiling stat too")
                ...
                n_llps_pass_geometry_weighted+=weight_llp_all[j]

                # Check if llp passes met cut
                met_llp = met_llp_all[j]
#                print("!!!!!!!!!!!!! met of llp that passes geometry cut = ", met_llp)
                if(met_llp > min_met):
                    n_llps_pass_geometry_met+=1
                    n_llps_pass_geometry_met_weighted+=weight_llp_all[j]                
                    ...
                    deltaR_llp_alljets = []
                    for jet_index in range(len(event_number_jets_all)):
                        start_smallloop = time.time()
                        if (event_number_jets_all[jet_index] == event_number_llp_all[j]):
#                            print("found jets in the same event, number ", data_llps['event_number_llp_all'][j])
                            ...
                            jet_x, jet_y, jet_z = ceil_intersection(eta_jets_all[jet_index], phi_jets_all[jet_index], nominal_jet_production_vertex)

                            # Toby+Jon's functions often assume that the jets are coming from LLPs?

                            # Calculate jet momentum
                            jet_p = pt_jets_all[jet_index] / np.sin(to_theta(eta_jets_all[jet_index]))
                            if (jet_p > jet_min_p):
                            ...
                            deltaeta_llp_thisjet = abs(eta_llp_all[j] - eta_jets_all[jet_index])
                            deltaphi_llp_thisjet = abs(phi_llp_all[j] - phi_jets_all[jet_index])
...
#                        print("Passed geometry+MET+deltaRjets")
                        ...
                        n_llps_pass_geometry_met_deltaRjets_weighted+=weight_llp_all[j]
                        ...
                        for charged_index in range(len(event_number_charged_all)):
                            if (event_number_charged_all[charged_index] == event_number_llp_all[j]):
#                                print("found charged energetic particles in the same event, number ", data_llps['event_number_llp_all'][j])

                                deltaeta_llp_thischarged = abs(eta_llp_all[j] - eta_charged_all[charged_index])
                                deltaphi_llp_thischarged = abs(phi_llp_all[j] - phi_charged_all[charged_index])
                        ...
                        if (sum(deltaR_llp_allcharged) == 0): # the llp is far enough away from all charged particles
                            ...
                            n_llps_pass_geometry_met_deltaRjets_deltaRcharged_weighted+=weight_llp_all[j]
    ...
    if save_cutflow:
        ...
        csv_out_dir = f"output_muons/{SSX}"
    ...
    if plot_decay_positions:

        # Cavern curvature (plotted as a circular arc)
        theta = np.linspace(0, np.pi, 100) #-np.pi / 2, np.pi / 2, 100)  # Half-circle for curvature
        curvature_x = cavern_r_curvature * np.cos(theta)
        curvature_y = cavern_r_curvature * np.sin(theta) + (cavern_sagitta + cavern_y_max - cavern_r_curvature)

        cavern_y_max_curvature = max(curvature_y)
#        print('max point in y from curvature: ', max(curvature_y))
#        print('max point in x from curvature: ', max(curvature_x))
        x_prod, y_prod, z_prod = zip(*xyz_production_vertex_llps)
#        x_dec, y_dec, z_dec = zip(*xyz_end_vertex_llps)
        x_dec, y_dec, z_dec = zip(*xyz_end_vertex_llps_cavern)

#        print(f"xyz end vertex llps: {xyz_end_vertex_llps}")

        plt.figure(figsize=(12, 7))
        ax = plt.gca()

        for y_p, z_p, y_d, z_d in zip(y_prod, z_prod, y_dec, z_dec):
            ax.plot([z_p, z_d], [y_p, y_d], marker='x',color='r')

        ax.axhline(y=cavern_y_max_curvature, color='green', linestyle='--', label='Cavern y max')
        ax.axhline(y=atlas_t_max, color='blue', linestyle='--', label='ATLAS max')
        ax.axvline(x=cavern_z_max, color='red', linestyle='--', label='Cavern z max/min')
        ax.axvline(x=-cavern_z_max, color='red', linestyle='--')
        ...
        plt.title(f"mass = {llp_mass}, coupling = {llp_Vmu} HNLs that decay in cavern: exponentially decayed HNL tracks")

        plt.figure(figsize=(12, 7))
        ax = plt.gca()

        for x_p, y_p, x_d, y_d in zip(x_prod, y_prod, x_dec, y_dec):
            ax.plot([x_p, x_d], [y_p, y_d], marker='x',color='r')

        # Cavern rectangular bounds
        ax.axhline(y=cavern_y_max_curvature, color='green', linestyle='--', label='Cavern y max')
        ax.axhline(y=atlas_t_max, color='blue', linestyle='--', label='ATLAS max')
        ax.axvline(x=-cavern_x_max, color='red', linestyle='-.', label='Cavern x max/min')
        ax.axvline(x=cavern_x_max, color='red', linestyle='-.')

        ax.plot(curvature_x, curvature_y, color='purple', linestyle='-', label='Cavern curvature')
        ...
        plt.title(f"xy plane for mass = {llp_mass}, coupling = {llp_Vmu}: exponentially decayed HNL tracks")
            
        # for checks / plotting:
#        filtered_decay_pos = [list(decay) for decay in xyz_end_vertex_llps if is_within_cavern(decay)]
#        print(f"filtered decay pos (in cavern): {filtered_decay_pos}")
