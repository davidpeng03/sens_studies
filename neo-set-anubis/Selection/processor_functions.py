...
def calculate_NLLPsinparents(p, llp_pid):
    ...
    def check_parents(parents, generations_left):
        nonlocal NLLPsinparents
        if generations_left <= 0:  # if wereached the max generations then stop
            return
        
        for parent in parents:
            if parent.pid == llp_pid:
                NLLPsinparents += 1
            else:
                check_parents(parent.parents, generations_left - 1)

    check_parents(p.parents, max_generations)
